...
export async function proxy(request: NextRequest) {
  ...
  // Лог для перевірки в панелі Vercel Logs
  console.log("Proxy handling path:", pathname);

  // 1. Якщо запит на авторизацію або API — робимо rewrite
  if (pathname.startsWith("/auth") || pathname.startsWith("/api")) {
    const targetUrl = new URL(pathname + request.nextUrl.search, BACKEND_URL);
    return NextResponse.rewrite(targetUrl);
  }

  // 2. Логіка захисту роутів (Redirect якщо не авторизований)
  const accessToken = request.cookies.get("accessToken")?.value;
  if (
    ["/profile", "/notes"].some((r) => pathname.startsWith(r)) &&
    !accessToken
